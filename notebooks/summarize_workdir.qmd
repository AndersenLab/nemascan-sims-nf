---
title: "Nextflow Work Directory Audit"
date: today
format:
  html:
    toc: true
...
Summarizes per-task disk usage metrics produced by `scripts/scan_workdir.sh`. Each section
writes output files to `out_dir` and displays results inline.

**Inputs:**

| Parameter | Value |
|-----------|-------|
| `tsv_in` | `r params$tsv_in` |
| `out_dir` | `r params$out_dir` |
...
tsv_in  <- params$tsv_in
out_dir <- params$out_dir

if (!nzchar(tsv_in)) {
  stop("params$tsv_in must be set. Run: quarto render notebooks/summarize_workdir.qmd -P tsv_in:<path>")
}
if (!file.exists(tsv_in)) {
  stop("File not found: ", tsv_in)
}
...
save_plot <- function(p, name, width = 10, height = 7) {
  ...
  ggsave(file.path(out_dir, paste0(name, ".png")), p, width = width, height = height, dpi = 150)
  invisible(p)
}
```
...
**`r nrow(df)` task dirs loaded** across **`r n_distinct(df$process)` processes**.
Panel sizes found: `r paste(sort(na.omit(unique(df$panel_size_n))), collapse = ", ")`.
Total data volume: **`r round(sum(df$total_gb, na.rm = TRUE), 1)` GB**.
...
process_summary |>
  ...
  kable(caption = "Per-process disk usage summary")
```

### Per process × panel size
...
panel_summary |>
  mutate(across(where(is.double), \(x) round(x, 3))) |>
  kable(caption = "Per process × panel size summary (key processes only)")
```
...
### Total GB per process

```{r}
#| label: plot-total-usage
#| fig-height: 8
...
### Task-level size distributions (top 10 processes)
...
if (nrow(p3_data) > 0) {
  ...
  message("No panel size data for key GWAS processes — skipping plot.")
}
```

### Symlink fraction per process

```{r}
#| label: plot-symlink-fraction
...
### Dominant file type per process
...
if (nrow(p5_data) > 0) {
  ...
  message("No file-type data available — skipping plot.")
}
```

### Total GB stacked by process + panel size

```{r}
#| label: plot-total-by-panel
...
if (nrow(p6_data) > 0) {
  ...
  message("No panel size data for key processes — skipping plot.")
}
```
...
process_summary |>
  slice_head(n = 10) |>
  mutate(
    total_gb_sum = round(total_gb_sum, 2),
    median_gb    = round(median_gb, 3)
  ) |>
  select(process, phase, n_tasks, median_gb, total_gb_sum, top_ext) |>
  kable(
    col.names = c("Process", "Phase", "N tasks", "Median GB", "Total GB", "Top ext"),
    caption   = "Top 10 processes by total disk usage"
  )
```
