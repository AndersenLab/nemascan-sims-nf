...
date: today
params:
  title: "Integration Cross-Validation Failures"
  nf_version: "version 24.10.4, build 5934 (20-01-2025 16:47 UTC)"
  executor: "local"
  command: "bash tests/collect_test_data.sh --clean"
  error: "Multiple tests errors and failures in `test-cross_validation.R` and `test-db-structure.R`"
  config: "test,docker"
  context: ""
format: gfm
engine: knitr
...
Errors and failures observed during integration tests, specifically in `test-cross_validation.R` and `test-db-structure.R`. 
...
Integration tests run locally with Docker, using the `test` profile which includes the `docker` config.
