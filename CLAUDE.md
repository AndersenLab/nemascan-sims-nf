...
- GCTA phenotype simulation can produce very small variance; `GCTA_MAKE_GRM` runs an iterative REML loop that scales phenotypes ×1000 per round until Vp >= 1e-4 (max 4 rounds). `GCTA_PERFORM_GWA` has no convergence guard — REML failures with the full GRM are expected-never for valid phenotypes that passed the `GCTA_MAKE_GRM` guard; a hard task failure with Nextflow retry is the correct response if one occurs.
