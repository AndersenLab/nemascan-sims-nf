#!/usr/bin/env bash
#
# Generate the C. tropicalis test VCF for nemascan-sims-nf integration testing.
#
# Subsets a CaeNDR C. tropicalis isotype VCF to a small set of strains and
...
#   - bcftools (>= 1.16)
#   - tabix (from htslib)
#   - Source VCF: CT.YYYYMMDD.hard-filter.isotype.vcf.gz
#     Download from CaeNDR: https://caendr.org/data/release/latest
#
# Usage:
#   ./generate_test_ct_vcf.sh <source_ct_vcf> [strain_list_file]
#
#   source_ct_vcf   Path to CT.YYYYMMDD.hard-filter.isotype.vcf.gz
#   strain_list_file  Optional: one strain name per line.
#                     Default: first 20 strains listed in the source VCF.
#
# Output:
#   data/test/test_ct.vcf.gz             (BGZF-compressed VCF)
#   data/test/test_ct.vcf.gz.tbi         (tabix index)
#   data/test/test_ct.YYYYMMDD.vcf.gz    (dated symlink → test_ct.vcf.gz)
#   data/test/test_ct.YYYYMMDD.vcf.gz.tbi (dated symlink → test_ct.vcf.gz.tbi)
#
# On completion the script writes the three C. tropicalis rows directly into
# data/test/test_strains_three_species.txt, replacing any existing ct.* rows.
# Rows for other species (ce.*, cb.*) are preserved unchanged.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTPUT_VCF="${SCRIPT_DIR}/test_ct.vcf.gz"
CHROMOSOMES="I,II,V"
...
# --- Parse arguments ---
if [[ $# -lt 1 ]]; then
    echo "Usage: $0 <source_ct_vcf> [strain_list_file]"
    echo ""
    echo "  source_ct_vcf   Path to CT.YYYYMMDD.hard-filter.isotype.vcf.gz"
...
if [[ -z "$DATE" ]]; then
    ...
    echo "Rename the file to include a YYYYMMDD date (e.g. CT.20221202.hard-filter.isotype.vcf.gz)"
    exit 1
fi
...
TEMP_VCF=$(mktemp "${SCRIPT_DIR}/test_ct_XXXXXX.vcf.gz")
bcftools view \
    -S "$STRAIN_LIST_FILE" \
    -t "$CHROMOSOMES" \
    --min-ac 1 \
...
SAMPLED_VCF=$(mktemp "${SCRIPT_DIR}/test_ct_sampled_XXXXXX.vcf.gz")
...
SYMLINK_VCF="${SCRIPT_DIR}/test_ct.${DATE}.vcf.gz"
SYMLINK_TBI="${SCRIPT_DIR}/test_ct.${DATE}.vcf.gz.tbi"

# Remove existing symlinks with this date if they exist
rm -f "$SYMLINK_VCF" "$SYMLINK_TBI"

ln -s test_ct.vcf.gz     "$SYMLINK_VCF"
ln -s test_ct.vcf.gz.tbi "$SYMLINK_TBI"
echo "Created symlinks:"
echo "  $SYMLINK_VCF → test_ct.vcf.gz"
echo "  $SYMLINK_TBI → test_ct.vcf.gz.tbi"
...
echo ""
echo "=== C. tropicalis test VCF generated ==="
...

{
    ...
    # Preserve non-ct rows from existing strainfile
    if [[ -f "$THREE_SPECIES_FILE" ]]; then
        awk -F'\t' 'NR>1 && $1 !~ /^ct\./' "$THREE_SPECIES_FILE"
    fi
    printf "ct.test\tc_tropicalis\tdata/test/test_ct.%s.vcf.gz\t0.05\t0.8\t%s\n" \
        "$DATE" "$STRAINS_CSV"
    printf "ct.test.popA\tc_tropicalis\tdata/test/test_ct.%s.vcf.gz\t0.05\t0.8\t%s\n" \
        "$DATE" "$POPA_CSV"
    printf "ct.test.popB\tc_tropicalis\tdata/test/test_ct.%s.vcf.gz\t0.05\t0.8\t%s\n" \
        "$DATE" "$POPB_CSV"
} > "${THREE_SPECIES_FILE}.tmp"
mv "${THREE_SPECIES_FILE}.tmp" "$THREE_SPECIES_FILE"
echo "Updated $THREE_SPECIES_FILE (ct.test, ct.test.popA, ct.test.popB)"

echo "Remember to:"
echo "  1. git add data/test/test_ct.${DATE}.vcf.gz data/test/test_ct.${DATE}.vcf.gz.tbi"
echo "  2. git add data/test/test_strains_three_species.txt"
echo "  3. Update CT_VCF_RELEASE in data/test/release_ids.sh to ${DATE}"
echo ""
echo "Done."
